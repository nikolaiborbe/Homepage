...
astro_image: /src/static/placeholder-image.png
svelte_image: static/placeholder-image.png
subject: Math
chapter: Chapter 1
---
...
<Math client:visible func={"\\left[ {{\\left( \\frac{x-1}{k}\\right)}^{r-1}}\\ {{\\left( \\frac{k-x+1}{k} \\right)}^{n-r+1}}-{{\\left( \\frac{x}{k} \\right)}^{r-1}}\\ {{\\left( \\frac{k-x}{k} \\right)}^{n-r+1}} \\right]+{{f}_{r-1:n}}(x)"}/>
